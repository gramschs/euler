...
* Sie können das **Euler-Verfahren** zur numerischen Lösung eines
  Anfangwertproblems in Python implementieren.
...
Übertragen auf unser Anfangswertproblem gilt für den Entwicklungspunkt $t_i$ die
Approximation bzw. Taylorploynom

$$x(t) = x(t_{i}) + \dot{x}(t_{i}) \cdot (t-t_{i}).$$

Jetzt können wir für die erste Ableitung $\dot{x}$ die rechte Seite der
Differentialgleichung einsetzen und erhalten
...
Wie gut die numerische Lösung eine exakte Lösung eines Anfangwertproblems
